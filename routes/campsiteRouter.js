        ...
        Campsite.findById(req.params.campsiteId)
            .then(campsite => {
                if (campsite && campsite.comments.id(req.params.commentId)) {
                    // Check if the logged user ID matches the comment author ID
                    if (campsite.comments.id(req.params.commentId).author.equals(req.user._id)) {
                        Campsite.updateOne(
                            { "_id": req.params.campsiteId, "comments._id": req.params.commentId },
                    ...
                    let err = new Error(`Campsite ${req.params.campsiteId} or Comment ${req.params.commentId} not found`);
                    err.status = 404;
                    return next(err);
                }
            })
